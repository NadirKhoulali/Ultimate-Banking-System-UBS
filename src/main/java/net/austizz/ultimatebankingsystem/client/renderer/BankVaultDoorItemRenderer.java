...
public class BankVaultDoorItemRenderer extends BlockEntityWithoutLevelRenderer {
    private static final ResourceLocation TEXTURE =
            ResourceLocation.fromNamespaceAndPath(UltimateBankingSystem.MODID, "textures/block/bank_vault_door.png");

    private BankVaultDoorBlockEntityModel model;

    public BankVaultDoorItemRenderer(BlockEntityRenderDispatcher dispatcher, EntityModelSet modelSet) {
    ...
    public void renderByItem(ItemStack stack,
                             ...
                             int packedLight,
                             int packedOverlay) {
        BankVaultDoorBlockEntityModel vaultModel = getModel();
        poseStack.pushPose();
    ...
    private BankVaultDoorBlockEntityModel getModel() {
        if (model == null) {
            model = new BankVaultDoorBlockEntityModel(
                    Minecraft.getInstance().getEntityModels().bakeLayer(BankVaultDoorBlockEntityModel.LAYER_LOCATION)
            );
        }
        return model;
    }
