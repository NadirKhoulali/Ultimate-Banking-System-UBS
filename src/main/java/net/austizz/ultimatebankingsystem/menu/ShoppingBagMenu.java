...
public class ShoppingBagMenu extends AbstractContainerMenu {
    ...
    private void loadFromBagItem(ItemStack bagStack, ItemStackHandler handler) {
        ...
        handler.deserializeNBT(empty);

        if (bagStack == null || bagStack.isEmpty()) {
            return;
        }
        CompoundTag root = bagStack.getTag();
        if (root == null || !root.contains(ShoppingBagDataKeys.BAG_DATA_KEY)) {
            return;
        }
        CompoundTag bagData = root.getCompound(ShoppingBagDataKeys.BAG_DATA_KEY).copy();
        bagData.putInt("Size", BAG_SLOT_COUNT);
        handler.deserializeNBT(bagData);
    }
    ...
    private void saveToBagItem(ItemStack bagStack) {
        ...
        CompoundTag bagData = ((ItemStackHandler) this.itemHandler).serializeNBT();
        if (bagData.isEmpty()) {
            bagStack.removeTagKey(ShoppingBagDataKeys.BAG_DATA_KEY);
            return;
        }
        bagStack.getOrCreateTag().put(ShoppingBagDataKeys.BAG_DATA_KEY, bagData);
    }
