...
public final class ModPayloads {
            ...
            switch (action) {
                case "set_slot_inventory", "set_slot" -> {
                    ...
                    if (!shelf.isCreativeShelf()) {
                        ...
                        if (!existingDisplay.isEmpty() && ItemStack.isSameItemSameTags(existingDisplay, selected)) {
                ...
                case "save_stock_inventory" -> {
                    ...
                    if (!ItemStack.isSameItemSameTags(configured, selectedStack)) {
                        success = false;
    ...
    private static ShelfSlotSummary buildSlotSummary(ShelfDisplayBlockEntity shelf, int absoluteSlot, int visibleSlotIndex) {
        ...
        String itemId = "";
        if (configured) {
            ResourceLocation key = net.minecraftforge.registries.ForgeRegistries.ITEMS.getKey(item.getItem());
    ...
    private static void takeRegularDisplayItem(ServerPlayer player, ShelfDisplayBlockEntity shelf, int slot) {
        ...
        player.sendSystemMessage(moneyLiteral("§aPicked up ").append(give.getHoverName())
                .append(moneyLiteral("§a from display slot " + (slot + 1) + ".")));
    }
    ...
    private static void placeRegularDisplayItem(ServerPlayer player,
        ...
        if (configured.isEmpty()) {
            ...
            player.sendSystemMessage(moneyLiteral("§aPlaced ").append(placed.getHoverName())
                    .append(moneyLiteral("§a into display slot " + (slot + 1) + ".")));
            return;
        }
        player.sendSystemMessage(moneyLiteral("§eDisplay slot " + (slot + 1)
                + " already has an item. Take it back first, then place a new one."));
    }
