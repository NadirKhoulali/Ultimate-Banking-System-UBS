...
public class ShelfItemPositionScreen extends Screen {
    ...
    public ShelfItemPositionScreen(ShelfOpenPayload payload, ShelfUnitSummary shelf, ShelfSlotSummary slot) {
        super(Component.literal("Shelf Item Transform"));
    ...
    private void addValueFields() {
        for (int i = 0; i < valueFields.length; i++) {
            ...
            DesktopEditBox editBox = new DesktopEditBox(
                    ...
                    valueFieldWidth,
                    valueFieldHeight,
                    Component.literal(valueLabels[i])
            );
    ...
    private Float parseField(int index, float fallback) {
        ...
        try {
            ...
            setFeedback(false, "Invalid numeric value in " + valueLabels[index] + ".");
            return null;
        }
    }
    ...
    public void render(GuiGraphics graphics, int mouseX, int mouseY, float partialTick) {
        ...
        if (hovered != null) {
            graphics.renderTooltip(font, Component.literal(hoverTooltip(hovered)), mouseX, mouseY);
        }
    }
    ...
    private void drawRightPanelText(GuiGraphics graphics) {
        ...
        for (int i = 0; i < valueLabels.length; i++) {
            graphics.drawString(font, valueLabels[i], labelX, fieldY(i) + 4, 0xFFE1EEFF, true);
        }
    }
