...
public class BankOwnerPcScreen extends Screen {
    ...
    public boolean mouseScrolled(double mouseX, double mouseY, double scrollX, double scrollDelta) {
        ...
        return super.mouseScrolled(localMouseX, localMouseY, scrollX, scrollDelta);
    }
    ...
    public void render(GuiGraphics graphics, int mouseX, int mouseY, float partialTicks) {
        ...
        graphics.drawString(this.font, tr("UBS Commerce Desktop"), left + 10, top + 9, 0xFF1E324E, false);
    ...
    private void drawDesktopHints(GuiGraphics graphics) {
        ...
        graphics.drawString(this.font, tr("Desktop Apps"), x, y, 0xFFFFFFFF, false);
    ...
    private void drawInputHelpPanel(GuiGraphics graphics,
        ...
        graphics.drawString(this.font, tr("Input Assistant"), x + 8, y + 9, 0xFFFFFFFF, false);
        ...
        if (lineY <= y + height - 20) {
            graphics.drawString(this.font, tr("Example: ") + fitToWidth(help.example(), width - 66), x + 8, lineY + 4, 0xFF98E2AF, false);
        }
    }
    ...
    private void drawShopLevelRoadmapModal(GuiGraphics graphics,
        ...
        graphics.drawString(this.font, tr("Level ") + node.level() + tr(" Unlock Details"), modalX + 8, modalY + 7, 0xFFFFFFFF, false);
    ...
    private void drawShopTrendChart(GuiGraphics graphics,
        ...
        graphics.fill(x - 1, y - 1, x + width + 1, y + height + 1, 0xFF355474);
        graphics.fill(x, y, x + width, y + height, 0x75192D45);
        graphics.drawString(this.font, tr("Sales Trend (Last 7 Days)"), x + 6, y + 6, 0xFFE5F3FF, false);
        ...
        if (trend.size() < 2) {
            graphics.drawString(this.font, tr("Not enough history yet."), plotX1 + 4, plotY1 + 4, 0xFFC9DEF3, false);
            return;
        }
    ...
    private void drawShopCategoryChart(GuiGraphics graphics,
        ...
        graphics.fill(x - 1, y - 1, x + width + 1, y + height + 1, 0xFF355474);
        graphics.fill(x, y, x + width, y + height, 0x75192D45);
        graphics.drawString(this.font, tr("Sales by Category (Value)"), x + 6, y + 6, 0xFFE5F3FF, false);

        Map<String, Long> values = categories == null ? Map.of() : categories;
        if (values.isEmpty()) {
            graphics.drawString(this.font, tr("No category data yet."), x + 8, y + 22, 0xFFC9DEF3, false);
            return;
        }
    ...
    private int drawShopInventoryCards(GuiGraphics graphics,
                                       ...
                                       int height) {
        if (shelves == null || shelves.isEmpty()) {
            graphics.drawString(this.font, tr("No shelf slots match current view. Scan Shelves or change filter/sort."), x + 6, y + 6, 0xFFE6F3FF, false);
            return height;
        }
        ...
        for (int i = 0; i < shelves.size(); i++) {
            ...
            List<ShopInventoryItemCardData> items = shelf.items() == null ? List.of() : shelf.items();
            if (items.isEmpty()) {
                graphics.drawString(this.font, tr("No configured items in this shelf."), cardX + 8, itemY + 6, 0xFFB2CAE3, false);
                continue;
            }
    ...
    private void drawOrderBoardApp(GuiGraphics graphics, int x, int y, int width, int height) {
        ...
        if (cards.isEmpty()) {
            graphics.drawString(this.font, tr("No open delivery orders right now."), viewX + 8, cursorY + 6, 0xFFD1E8FF, false);
        } else {
            int cardH = 66;
            int cardGap = 6;
            int cardX = viewX + 4;
    ...
    private int getWebshopContentHeight(int width, int viewportHeight, List<String> lines) {
        ...
        y += 14 + (orderRows * 68) + (Math.max(0, orderRows - 1) * 6) + sectionGap;
    ...
    private void drawWebshopApp(GuiGraphics graphics, int x, int y, int width, int height) {
        ...
        if (orders.isEmpty()) {
            ...
            for (WebshopOrderCardData card : orders) {
                ...
                String issueLine = isBlankWebshopOrderError(card.lastError())
                        ? ("Attempts " + card.attempts())
                        : ("Last issue " + card.lastError());
                graphics.drawString(this.font, fitToWidth(issueLine, orderCardW - 12), cardX + 6, cardY + 54,
                        isBlankWebshopOrderError(card.lastError()) ? 0xFF8EACC7 : 0xFFFFC8A8, false);
    ...
    private void drawWebshopCardHoverTooltip(GuiGraphics graphics, int mouseX, int mouseY) {
        ...
        if (title == null) {
            for (WebshopOrderCardHitbox hitbox : visibleWebshopOrderCards) {
                ...
                if (!isBlankWebshopOrderError(card.lastError())) {
                    lines.add("Last issue: " + card.lastError());
    ...
    private int drawShopEmployeeCards(GuiGraphics graphics,
        ...
        if (employees == null || employees.isEmpty()) {
            graphics.drawString(this.font, tr("No employees found."), x + 6, y + 6, 0xFFE6F3FF, false);
            return height;
        }
    ...
    private void drawShopFinanceSnapshot(GuiGraphics graphics,
                                         ...
                                         int y,
                                         int width) {
        graphics.drawString(this.font, tr("Finance Overview"), x + 4, y + 2, 0xFFE8F4FF, false);
    ...
    private void drawShopVaultSnapshot(GuiGraphics graphics,
                                       ...
                                       int y,
                                       int width) {
        graphics.drawString(this.font, tr("Cash Vault (Exact Legal Tender)"), x + 4, y + 2, 0xFFE8F4FF, false);
        ...
        graphics.drawString(this.font, tr("Denomination | Count | Subtotal"), x + 6, listY + 6, 0xFFE6F3FF, false);
        int lineY = listY + 18;
        ...
        if (vault.counts().isEmpty()) {
            graphics.drawString(this.font, tr("No legal tender is stored in the vault yet."), x + 6, lineY, 0xFFCFE4F8, false);
        }
    }
    ...
    private int drawShopOwnerAccountCards(GuiGraphics graphics,
        ...
        if (cards == null || cards.isEmpty()) {
            graphics.drawString(this.font, tr("No owner accounts available."), x + 6, y + 6, 0xFFE6F3FF, false);
            return height;
        }
        ...
        for (int i = 0; i < cards.size(); i++) {
            ...
            graphics.drawString(this.font,
                    ...
                    0xFFB2CAE3,
                    false);
            graphics.drawString(this.font, selected ? tr("Selected") : tr("Click to select"), cardX + 8, cardY + 58,
    ...
    private int drawShopPermissionRoleCards(GuiGraphics graphics,
        ...
        if (safeHeaders.isEmpty()) {
            ...
            graphics.drawString(this.font, tr(line), x + 6, y + 6, 0xFFE6F3FF, false);
            return height;
        }
        ...
        for (ShopPermissionRoleHeaderData header : safeHeaders) {
            ...
            if (groupMembers.isEmpty()) {
                graphics.drawString(this.font, tr("No players in this group."), x + 8, cursorY + 2, 0xFF9FBAD3, false);
                cursorY += 18 + cardGap;
                continue;
            }
            ...
            for (int i = 0; i < groupMembers.size(); i++) {
                ...
                if (!card.guest() && !card.owner() && card.grantedAtMillis() > 0L) {
                    ...
                    graphics.drawString(this.font,
                            selected ? tr("Selected • Click role + update/remove") : tr("Click to select"),
                            cardX + 8,
                            cardY + 59,
    ...
    private int drawShopVaultPlanEditor(GuiGraphics graphics,
        ...
        for (int i = 0; i < SHOP_CASH_DENOMINATIONS.length; i++) {
            ...
            graphics.drawString(this.font, tr("Selected: " + req), cardX + 8, cardY + 22, 0xFFCFE4F8, false);
    ...
    private void drawCreateWindowFrame(GuiGraphics graphics) {
        ...
        graphics.drawString(this.font, tr("Create New Player Bank"), left + 8, top + 10, 0xFFFFFFFF, false);
        graphics.drawString(this.font,
                tr("Owned: ") + ClientOwnerPcData.getOwnedCount() + " / " + ClientOwnerPcData.getMaxBanks(),
                left + 230,
                top + 10,
                0xFFE8F2FF,
                false);

        graphics.drawString(this.font, tr("Ownership Type"), left + 8, top + 56, 0xFF1D2F4A, false);
        graphics.drawString(this.font,
                tr("Selected: ") + tr(prettifyOwnership(selectedOwnershipModel)),
                left + 8,
                top + 68,
                0xFF2A496E,
                false);
    ...
    private void drawCreateShopWindowFrame(GuiGraphics graphics) {
        ...
        graphics.drawString(this.font, tr("Create New Shop"), left + 8, top + 10, 0xFFFFFFFF, false);
        graphics.drawString(this.font,
                tr("Owned: ") + countOwnedShopApps() + " / " + MAX_SHOPS_PER_PLAYER,
                left + 230,
                top + 10,
                0xFFE8F2FF,
                false);

        graphics.drawString(this.font, tr("Shop Type"), left + 8, top + 56, 0xFF1D2F4A, false);
        graphics.drawString(this.font,
                tr("Selected: ") + tr(prettifyShopType(selectedShopType)),
                left + 8,
                top + 68,
                0xFF2A496E,
                false);
        graphics.drawString(this.font, tr("Description"), left + 8, top + 84, 0xFF1D2F4A, false);
    ...
    private void drawUtilityWindowFrame(GuiGraphics graphics) {
        ...
        if (unsavedClosePromptOpen) {
            ...
            graphics.drawString(this.font, tr("Unsaved changes"), modalX + 8, modalY + 6, 0xFFFFFFFF, false);
            String label = unsavedCloseTarget == UtilityApp.PAINT ? "Paint" : "Notepad";
            graphics.drawString(this.font, tr("Save ") + tr(label) + tr(" before closing?"), modalX + 8, modalY + 30, 0xFFD5E9FF, false);
        }
    }
    ...
    private void drawNotepadApp(GuiGraphics graphics, int x, int y, int width, int height) {
        ...
        if (notepadSaveModalOpen) {
            ...
            graphics.fill(modalX, modalY, modalX + modalW, modalY + modalH, 0xF01A3049);
            graphics.fill(modalX, modalY, modalX + modalW, modalY + 20, 0xE6285A8B);
            graphics.drawString(this.font, tr("Save Notepad"), modalX + 8, modalY + 6, 0xFFFFFFFF, false);
            graphics.drawString(this.font, tr("Enter a file name:"), modalX + 10, modalY + 26, 0xFFD5E9FF, false);
        }
    }
    ...
    private void drawFileExplorerApp(GuiGraphics graphics, int x, int y, int width, int height) {
        ...
        graphics.drawString(
                this.font,
                tr("Click a file card to open it."),
                innerX,
                infoY + 11,
                0xFF9EF0B6,
                false
        ...
        List<OwnerPcFileEntry> files = ClientOwnerPcData.getDesktopFiles();
        if (files.isEmpty()) {
            graphics.drawString(this.font, tr("No files saved on this PC yet."), explorerFileListX + 8, explorerFileListY + 8, 0xFF9FC2E6, false);
        }
    }
    ...
    private void drawPaintApp(GuiGraphics graphics, int x, int y, int width, int height) {
        ...
        if (paintSaveModalOpen) {
            ...
            graphics.fill(modalX, modalY, modalX + modalW, modalY + modalH, 0xF01A3049);
            graphics.fill(modalX, modalY, modalX + modalW, modalY + 20, 0xE6285A8B);
            graphics.drawString(this.font, tr("Save Canvas"), modalX + 8, modalY + 6, 0xFFFFFFFF, false);
            graphics.drawString(this.font, tr("Enter a file name:"), modalX + 10, modalY + 26, 0xFFD5E9FF, false);
        }
    }
    ...
    private void drawSystemMonitorApp(GuiGraphics graphics, int x, int y, int width, int height) {
        if (systemHideAppsMenuOpen) {
            ...
            graphics.drawString(this.font, tr("App Visibility"), panelX + 8, panelY + 8, 0xFFE4F2FF, false);
            graphics.drawString(this.font, tr("Click a card to toggle hidden/visible."), panelX + 8, panelY + 19, 0xFFBFD6EE, false);
            ...
            for (AppVisibilityCard card : visibleSystemAppCards) {
                ...
                graphics.drawString(this.font, tr(status), card.x() + 8, card.y() + card.height() - 11, hidden ? 0xFFFFC7C7 : 0xFFC7FFE0, false);
            }
            return;
        }
